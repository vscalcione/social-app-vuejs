    ...
    <v-row v-if="scream">
      ...
      <v-col cols="12" sm="8">
        <v-row>
          ...
          <v-col cols="12">
            <h2 class="title cyan--text hover" @click="getUser">
              {{ scream.userHandle }}
            </h2>
          ...
          <v-col cols="12" md="8" class="relative">
            <v-card-actions>
              ...
              <v-btn
                text
                @click="likeScream(isAuthenticated, scream)"
                small
                color="#2c003e"
              >
          ...
          <!--------------------------- SCREAM DELETE MODAL -------------------------->
          <div
            class="center ml-5 pa-0"
            v-if="isAuthenticated && userCredentials"
          >
            <AppDeleteScream
              v-show="userCredentials.handle === scream.userHandle"
              :scream="scream"
            ></AppDeleteScream>
          </div>
  ...
  computed: {
    ...
    isLiked() {
      if (this.isAuthenticated && this.userLikes) {
        let findIfScreamIsLiked = this.userLikes.findIndex(
